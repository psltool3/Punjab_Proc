                    ...
                    <div class="row">
                        <div class="col-md-12">
							...
							<div class="panel-heading">
                                    <h3 class="panel-title">Mill Data</h3>
                                </div>
                                    ...
                                    <table id="export_table" class="table datatable">
                                        <thead>
                                            <tr>
												...
												<th style="font-size:15px">Mill District</th>
												<th style="font-size:15px">Milling Centre</th>
												<th style="font-size:15px">Mill Name</th>
												<th style="font-size:15px">Mill ID</th>
												<th style="font-size:15px">Mill Latitude</th>
												<th style="font-size:15px">Mill Longitude</th>
												<th style="font-size:15px">Milling Process</th>
										...
										while($row = mysqli_fetch_array($result))
										{
											...
											"<td>{$row['district']}</td>".
											"<td>".($row['milling_center'] ?? '')."</td>".
											"<td>{$row['name']}</td>".
											"<td>".($row['mill_id'] ?? '')."</td>".
											"<td>{$row['latitude']}</td>".
											"<td>{$row['longitude']}</td>".
											"<td>".($row['milling_process'] ?? '')."</td>".
											"<td>{$row['active']}</td></tr>";
										}
    ...
    <!-- START SCRIPTS -->
		...
		document.getElementById('downloadCSV').addEventListener('click', async function() {
			try {
				...
				downloadFile(csvBlob, 'Punjab_Mill_' + getDateString() + '.csv');
			} catch (error) {
		...
		document.getElementById('downloadXLSX').addEventListener('click', async function() {
			try {
				...
				downloadFile(excelBlob, 'Punjab_Mill_' + getDateString() + '.xlsx');
			} catch (error) {
		...
		document.getElementById('downloadPDF').addEventListener('click', async function() {
			try {
				...
				link.href = url;
				link.download = 'Punjab_Mill_' + getDateString() + '.pdf';
				link.click();
