	...
	if ($_FILES["file"]["size"] > 0) {
		...
		while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
			if($i>0){
                ...
                if (
					!isset($column[$mill_id]) ||
					!preg_match('/^[A-Za-z0-9]+$/', $column[$mill_id])
				) {
					echo "Error: Mill ID should not contain spaces or any special characters: " . ($column[$mill_id] ?? 'Missing');
					echo "<br>";
					$redirect = 0;
				}	
		...
		if ($_FILES["file"]["size"] > 0) {
			...
			while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
				if($i>0){
					...
					if($numrows_insert==0){
						writeLog("User ->" ." Mill Added -> ". $_SESSION['user'] . "| " . $Mill->getName());
						$query_add = $Mill->insert($Mill);
						mysqli_query($con, $query_add);
					}
					else{
						echo "Error : Mill with ID ".$Mill->getMillId()." Already Exist</br>";
						$redirect = 2;
					}
				}
				else{
